...
namespace Valve.VR.InteractionSystem
{
	...
	public class CircularDrive : MonoBehaviour
	{
		...
		public RotSide rotSide = RotSide.BOTH;
		...
		private float lastRotAmt = 0;

		[HideInInspector]
		public float totRotAmt = 0;

		[Range(1,10)]
		public int maxRotCount = 1;//One rot amt equals 360

        private float    maxRotAmt = 360;
        private Vector3  startPnt;//Start Pos of bolt
        public Transform endTrans;//End pos of bolt

        public Transform clockwise;
        public Transform antiClockwise;
        public float     visualHintProxAmt = 10;//Used to show clockwise and anticlockwise arrow
        ...
        private void Start()
		{
            startPnt = transform.position;
            maxRotAmt = (float)maxRotCount * 360;
		...
		private void UpdateGameObject()
		{
			if ( rotateGameObject )
			{
                float diff = outAngle - lastRotAmt;
                lastRotAmt = outAngle;
                outAngle = Mathf.Clamp(outAngle, 0, maxRotAmt);
                bool applyRot = true;
                if (outAngle == 0)
                {
                    applyRot = false;
                }
                else if (outAngle == maxRotAmt)
                {
                    applyRot = false;
                }
                if(outAngle >= (maxRotAmt - visualHintProxAmt))
                {
                    antiClockwise.gameObject.SetActive(true);
                    clockwise.gameObject.SetActive(false);
                }
                else if (outAngle <= visualHintProxAmt)
                {
                    antiClockwise.gameObject.SetActive(false);
                    clockwise.gameObject.SetActive(true);
                }
                else
                {
                    antiClockwise.gameObject.SetActive(true);
                ...
                if (applyRot == false)
                    return;
                transform.position = Vector3.Lerp(startPnt, endTrans.transform.position, outAngle / maxRotAmt);
